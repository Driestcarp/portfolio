...
import { Mail, Github, FileUser } from "lucide-react";
...
const socials = [
  ...
  { icon: FileUser, label: "CV", href: personalInfo.cv },
];
          ...
          <div className="flex justify-center gap-4">
            {socials.map((social) => (
              <a
                ...
                className="group relative p-3 rounded-lg border border-border text-muted-foreground hover:text-primary hover:border-primary/50 transition-colors"
              >
                <social.icon className="w-5 h-5" />

                <span className="pointer-events-none absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-background border border-border px-2 py-1 text-xs text-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                  {social.label}
                </span>
              </a>
            ))}
